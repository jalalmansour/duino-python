...
from duino.lib._pydantic import to_strict_json_schema
...
def test_most_types() -> None:
    if not PYDANTIC_V1:
        assert duino.pydantic_function_tool(Query)["function"] == snapshot(
            {
                "name": "Query",
                "strict": True,
                "parameters": {
...
def test_enums() -> None:
    if not PYDANTIC_V1:
        assert duino.pydantic_function_tool(ColorDetection)["function"] == snapshot(
            {
                "name": "ColorDetection",
                "strict": True,
                "parameters": {
