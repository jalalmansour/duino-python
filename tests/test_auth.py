...
from duino import duino, OAuthError
from duino.auth._workload import (
...
@respx.mock
def test_basic_auth():
    respx.post("https://auth.duino.com/oauth/token").mock(
    ...
    respx.get("https://api.duino.com/v1/models").mock(
    ...
    assert token_call.request.url == "https://auth.duino.com/oauth/token"
...
def test_workload_identity_exchange_payload_and_cache() -> None:
    ...
    exchange_route = respx.post("https://auth.duino.com/oauth/token").mock(
        return_value=httpx.Response(
            200,
            json={
                "access_token": "fake_access_token",
    ...
    api_route = respx.get("https://api.duino.com/v1/models").mock(
        return_value=httpx.Response(200, json={"data": [], "object": "list"})
    )
