...
from ._exceptions import DuinoError, APIStatusError
...
__all__ = ["Timeout", "Transport", "ProxiesTypes", "RequestOptions", "Duino", "AsyncDuino", "Client", "AsyncClient"]
...
class Duino(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new synchronous Duino client instance.
        ...
        if admin_api_key is None:
            admin_api_key = os.environ.get("DUINO_ADMIN_KEY")
        ...
        if (
            ...
            raise DuinoError(
                "Missing credentials. Please pass an `api_key`, `workload_identity`, `admin_api_key`, or set the `DUINO_API_KEY` or `DUINO_ADMIN_KEY` environment variable."
            )

        if organization is None:
            organization = os.environ.get("DUINO_ORG_ID")
        self.organization = organization

        if project is None:
            project = os.environ.get("DUINO_PROJECT_ID")
    ...
    def default_headers(self) -> dict[str, str | Omit]:
        return {
            **super().default_headers,
            ...
            "Duino-Organization": self.organization if self.organization is not None else Omit(),
            "Duino-Project": self.project if self.project is not None else Omit(),
...
class AsyncDuino(AsyncAPIClient):
    # client options
    api_key: str
    admin_api_key: str | None
    workload_identity: WorkloadIdentity | None
