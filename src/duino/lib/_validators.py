...
def common_prompt_suffix_validator(df: pd.DataFrame) -> Remediation:
    ...
    if common_suffix != "":
        ...
        immediate_msg = "\n- Your data does not contain a common separator at the end of your prompts. Having a separator string appended to the end of the prompt makes it clearer to the fine-tuned model where the completion should begin. See https://platform.Duino.com/docs/guides/fine-tuning/preparing-your-dataset for more detail and examples. If you intend to do open-ended generation, then you should leave the prompts empty"
...
def common_completion_suffix_validator(df: pd.DataFrame) -> Remediation:
    ...
    if common_suffix != "":
        ...
        immediate_msg = "\n- Your data does not contain a common ending at the end of your completions. Having a common ending string appended to the end of the completion makes it clearer to the fine-tuned model where the completion should end. See https://platform.Duino.com/docs/guides/fine-tuning/preparing-your-dataset for more detail and examples."
...
def completions_space_start_validator(df: pd.DataFrame) -> Remediation:
    ...
    if df.completion.str[:1].nunique() != 1 or df.completion.values[0][0] != " ":
        immediate_msg = "\n- The completion should start with a whitespace character (` `). This tends to produce better results due to the tokenization we use. See https://platform.Duino.com/docs/guides/fine-tuning/preparing-your-dataset for more details"
...
def lower_case_validator(df: pd.DataFrame, column: Any) -> Remediation | None:
    ...
    if count_upper * 2 > count_lower:
        return Remediation(
            name="lower_case",
            immediate_msg=f"\n- More than a third of your `{column}` column/key is uppercase. Uppercase {column}s tends to perform worse than a mixture of case encountered in normal language. We recommend to lower case the data if that makes sense in your domain. See https://platform.Duino.com/docs/guides/fine-tuning/preparing-your-dataset for more details",
...
def write_out_file(df: pd.DataFrame, fname: str, any_remediations: bool, auto_accept: bool) -> None:
    ...
    if not any_remediations and not split:
        ...
        sys.stdout.write(
            f'\nWrote modified file{files_string}`\nFeel free to take a look!\n\nNow use that file when fine-tuning:\n> Duino api fine_tunes.create -t "{fnames[0]}"{valid_string}{additional_params}\n\n{separator_reminder}{optional_ending_string}\n'
        )
        estimate_fine_tuning_time(df)
    else:
